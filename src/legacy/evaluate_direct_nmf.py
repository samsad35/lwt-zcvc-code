...
def main():
    ...
    print("\n3. Running DIRECT NMF Pipeline (No Regression W)...")
    ...
    for src in tqdm(source_data, desc="Evaluating Direct-NMF"):
        X_src = src["x"]
        
        # OMP soft-alignment (Target Pool as dictionary)
        omp.fit(X_tgt_pool.T, X_src.T)
        H = omp.coef_  # Shape: (T, N_tgt)
        
        # Directly generate the converted frames by blending Target Frames!
        X_converted = np.matmul(H, X_tgt_pool)
        
        # Vocode directly without passing through a Linear W bottleneck
    ...
    print("\n================ DIRECT NMF RESULTS (No Regression) ================")
