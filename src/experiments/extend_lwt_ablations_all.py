...
print("================================================================================")
print("=== EXTENDING LWT ABLATIONS FOR BETA, K, AND ALPHA (DEGENERATION TAILS, N=200) ===")
print("================================================================================")
...
# Pre-extract target utterances cache (T=20 utts)
...
# Setup K=10 shared background model
K_fixed = 10
...
# --------------------------------------------------------------------------
# 3. EXTEND ALPHA ABLATION
# --------------------------------------------------------------------------
print("\n--- Extending Alpha Speaker Boost Ablation (K=10, Beta=20, T=20, N=200) ---")
csv_alpha = Path('/local_scratch/ssadok/un_projet_audio/output/tables/lwt_ablation_alpha_large_N.csv')
df_alpha_old = pd.read_csv(csv_alpha) if csv_alpha.exists() else pd.DataFrame()

# Degeneration tails for alpha:
# Lower tail: 0.0 (no speaker cov transferred), 0.2
# Upper tail: 2.5, 3.0 (extreme speaker covariance over-amplification)
new_alphas = [0.0, 0.2, 2.5, 3.0]
new_alpha_records = []
for a in new_alphas:
    t0 = time.time()
    boosted_cov = {}
    ...
    new_alpha_records.append({'Alpha': a, 'CER': cer, 'Sim': sim, 'Jitter': jit})
    print(f"  Alpha = {a:4.1f} | CER: {cer:5.2f}% | Sim: {sim:.4f} | Jitter: {jit:.3f} | ({time.time()-t0:.1f}s)")

df_alpha_all = pd.concat([df_alpha_old, pd.DataFrame(new_alpha_records)], ignore_index=True)
df_alpha_all = df_alpha_all.drop_duplicates(subset=['Alpha']).sort_values('Alpha').reset_index(drop=True)
df_alpha_all.to_csv(csv_alpha, index=False)
print(f"Saved extended Alpha ablation to {csv_alpha}")

print("\n[ALL COMPLETED] Extended all ablations (Beta, K, Alpha) successfully!")
