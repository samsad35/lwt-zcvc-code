...
def main():
    ...
    print("\n2. Loading Dataset & Computing Target GMM-WCT...")
    ...
    N_COMPONENTS = 128
    K_CLUSTERS = 5
    BETA = 2.0 # Softmax temperature for continuous interpolation
    
    print(f"   Training K-Means (K={K_CLUSTERS}) on Target Pool...")
    ...
    for k in range(K_CLUSTERS):
        ...
        # In case a cluster is too small, fallback gracefully
        if len(X_k) > 10:
    ...
    for p in tqdm(src_paths, desc="Evaluating GMM-WCT"):
        X_src, w_src = extract_features(p, wavlm_large)
        ref_text = transcribe(w_src)
        
        # 1. Global Whitening for Source (to maintain temporal structure without local noise)
        ...
        # 2. Soft Local Coloring (Frame by Frame Interpolation)
        X_colored = np.zeros_like(X_src)
        
        # Calculate distance of each source frame to target centroids
    ...
    print(f"\n================ SOFT GMM-WCT RESULTS (K={K_CLUSTERS}, Dim={N_COMPONENTS}) ================")
