...
def get_gp_wct_matrices(X_s, X_t_pool, k=1024):
    mu_s = np.mean(X_s, axis=0)
    mu_t = np.mean(X_t_pool, axis=0)
    
    C_s = np.cov(X_s - mu_s, rowvar=False)
    C_t = np.cov(X_t_pool - mu_t, rowvar=False)
    ...
    # 1. Whitening Source
    W_s = evecs_s @ np.diag(1.0 / np.sqrt(evals_s))
    
    # 2. Procrustes Alignment of Eigenvectors
    M = evecs_s.T @ evecs_t
    U, _, Vt = np.linalg.svd(M)
    R = U @ Vt  # Optimal orthogonal rotation
    
    # 3. Coloring Target
    C_t_mat = np.diag(np.sqrt(evals_t)) @ evecs_t.T
...
def main():
    ...
    print("\n2. Loading Dataset & Computing Global Pool...")
    ...
    N_COMPONENTS = 1024 # Global Pool allows full rank!
    print(f"\n3. Running Global Procrustean WCT (GP-WCT, Dim={N_COMPONENTS})...")
    ...
    for p in tqdm(src_paths, desc="Evaluating GP-WCT"):
        X_src, w_src = extract_features(p, wavlm_large)
        ref_text = transcribe(w_src)
        
        # Calculate GP-WCT Matrix
        k_src = min(N_COMPONENTS, X_src.shape[0] - 1)
        mu_s, mu_t, T = get_gp_wct_matrices(X_src, X_tgt_pool, k=k_src)
        
        # Convert
        X_converted = np.matmul(X_src - mu_s, T) + mu_t
    ...
    print(f"\n================ GP-WCT RESULTS (Dim={N_COMPONENTS}) ================")
