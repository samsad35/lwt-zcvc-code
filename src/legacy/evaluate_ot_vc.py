...
def main():
    ...
    print("\n3. Running OPTIMAL TRANSPORT VC (EMD)...")
    ...
    N_tgt = X_tgt_pool.shape[0]
    b = np.ones((N_tgt,)) / N_tgt
    
    for p in tqdm(src_paths, desc="Evaluating OT-VC"):
        X_src, w_src = extract_features(p, wavlm_large)
        ref_text = transcribe(w_src)
        
        T_src = X_src.shape[0]
        a = np.ones((T_src,)) / T_src
        
        # Cost matrix: Cosine Distance
        C = cosine_distances(X_src, X_tgt_pool)
        
        # Solve Exact Optimal Transport
        # P shape: (T_src, N_tgt)
        P = ot.emd(a, b, C)
        
        # Reconstruct Source frames using the Transport Plan
        # We multiply by T_src so that the sum of weights for each source frame is 1
        X_aligned = np.matmul(P * T_src, X_tgt_pool)
        
        # Convert directly (Zero-Shot)
