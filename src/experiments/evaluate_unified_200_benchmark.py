...
print("================================================================================")
print("=== LAUNCHING UNIFIED BENCHMARK ON N=200 CONVERSIONS ACROSS ALL VC MODELS ===")
print("================================================================================")
...
print("Loading evaluation models (ASR Wav2Vec2-960h & Speaker ECAPA-TDNN)...")
...
src_files = src_files[:20] # exactly 20 source utterances
print(f"Loaded {len(target_speakers)} target speakers and {len(src_files)} source utterances.")
print(f"Total Unified Conversions per model = {len(target_speakers)} x {len(src_files)} = {len(target_speakers)*len(src_files)}")

# Pre-extract all source utterances and references
print("Pre-extracting source features and ground truth transcriptions...")
src_data = []
for p in src_files:
    feat, wav = ext(p)
    ground_truth = tr(wav)
...
# Precompute universal shared phonetic covariance from external background pool
print("Precomputing Population Shared Phonetic Covariances for K=10...")
...
# Precompute Target Models (T=20 utterances per target speaker)
T_target_utts = 20
target_models = {}
...
print("\nStarting unified conversion loop (7 methods x 200 conversions = 1400 evaluations)...")
for m_idx, (method_name, approach_type) in enumerate(methods):
    ...
    for tgt_spk, tm in target_models.items():
        for s_item in src_data:
            ...
            count += 1
            if count % 50 == 0:
                print(f"    Completed {count}/200 conversions...")
                
    elapsed_m = time.time() - t_start_method
    print(f"  --> {method_name} completed in {elapsed_m:.1f}s.")

df_detail = pd.DataFrame(detailed_records)
out_detail = Path('/local_scratch/ssadok/un_projet_audio/output/tables/unified_200_conversions_detail.csv')
df_detail.to_csv(out_detail, index=False)
print(f"\n[DONE] Saved 1,400 detailed conversion records to {out_detail}")

# Compute Benchmark Summary across exactly 200 conversions per method
...
print("\n================================================================================")
print("=== FINAL UNIFIED BENCHMARK SUMMARY (N=200 CONVERSIONS PER MODEL) ===")
print("================================================================================")
...
with open(tex_path, 'w') as f:
    ...
    f.write("    \\centering\n")
    f.write("    \\caption{Comprehensive performance comparison across all voice conversion paradigms evaluated on a strictly unified benchmark of $N=200$ test conversions (10 target speakers, 20 source utterances from 10 distinct speakers, $T=20$ target utterances).}\n")
