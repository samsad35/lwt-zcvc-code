...
def main():
    ...
    # Ablation Conditions
    # Axis 1: K in [1, 3, 5, 8, 16] with Soft (beta=2.0) vs Hard (argmax)
    # Axis 2: beta in [0.5, 1.0, 2.0, 5.0, 10.0] for K=5
    
    print("\n4. Running The Alignment Continuum Ablation...")
    
    K_LIST = [1, 3, 5, 8, 16]
    REGIMES = ["Soft (beta=2.0)", "Hard (Argmax)"]
    ...
    for K in K_LIST:
        ...
        if K == 1:
            ...
            print(f"  [K=1 | Classic WCT] CER: {avg_cer:.2f}% | Sim: {avg_sim:.4f}")
            results_continuum.append({"K": 1, "Regime": "Global (K=1)", "CER (%)": avg_cer, "Similarity": avg_sim})
            continue
        ...
        for k in range(K):
            ...
            if len(X_k) > 10:
                ...
                mu_k, _, C_k = get_wct_matrices(X_tgt_pool, k=N_COMPONENTS)
        ...
        for regime in REGIMES:
            ...
            for X_src, ref_text, _ in source_data:
                ...
                if regime == "Hard (Argmax)":
                    # Hard assignment: each frame uses strictly its closest cluster
                    hard_idx = np.argmin(dists, axis=1)
                    weights = np.zeros_like(dists)
                    for i, idx in enumerate(hard_idx):
                        weights[i, idx] = 1.0
                else:
                    # Soft assignment with beta = 2.0
                    logits = -2.0 * dists
            ...
            avg_cer = np.mean(cers) * 100
            avg_sim = np.mean(sims)
            print(f"  [K={K} | {regime}] CER: {avg_cer:.2f}% | Sim: {avg_sim:.4f}")
            results_continuum.append({"K": K, "Regime": regime, "CER (%)": avg_cer, "Similarity": avg_sim})

    # Axis 2: Varying Temperature beta for K=5
    print("\n--- Temperature Ablation for K=5 ---")
    ...
    for k in range(5):
        X_k = X_tgt_pool[tgt_labels == k]
        mu_k, _, C_k = get_wct_matrices(X_k, k=N_COMPONENTS)
        cluster_matrices_5.append((mu_k, C_k))
        
    BETAS = [0.2, 0.5, 1.0, 2.0, 5.0, 10.0]
    results_temp = []
    ...
    for beta in BETAS:
        ...
        avg_cer = np.mean(cers) * 100
        avg_sim = np.mean(sims)
        print(f"  [K=5 | beta={beta}] CER: {avg_cer:.2f}% | Sim: {avg_sim:.4f}")
        results_temp.append({"beta": beta, "CER (%)": avg_cer, "Similarity": avg_sim})

    print("\n================== CONTINUUM ABLATION RESULTS ==================")
    df_res1 = pd.DataFrame(results_continuum)
    print(df_res1.to_string(index=False))
    
    print("\n================== TEMPERATURE ABLATION (K=5) ==================")
    df_res2 = pd.DataFrame(results_temp)
    print(df_res2.to_string(index=False))
    
    df_res1.to_csv("continuum_ablation_results.csv", index=False)
    df_res2.to_csv("temperature_ablation_results.csv", index=False)
