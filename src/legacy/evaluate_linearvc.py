...
def main():
    ...
    tgt_embs = [get_embedding(w) for w in tgt_wavs[:10]] # Average 10 for reference profile
    tgt_profile = torch.stack(tgt_embs).mean(dim=0)
    tgt_profile = torch.nn.functional.normalize(tgt_profile, dim=0)

    # 25 Random Source utterances (same exact sampling seed as our CCA script!)
    ...
    print("\n3. Running Pure LinearVC Metrics Pipeline...")
    cers = []
    sims = []
    
    for src in tqdm(source_data, desc="Evaluating LinearVC"):
        X_src = src["x"]
        
        # 3.1 kNN Alignment in Entangled WavLM Space
        ...
        # 3.2 Learn W_tgt for this specific utterance (Zero-Shot shortcut for LinearVC)
