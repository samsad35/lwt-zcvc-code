...
def compute_effective_rank(matrix):
    ...
    _, S, _ = torch.linalg.svd(t_mat, full_matrices=False)
    S = S.cpu().numpy()
...
def run_rank_experiment(
    ...
    custom_palette = sns.color_palette("husl", len(models_config))
    ...
    ax.set_title("Évolution du Rang Effectif de l'Espace Latent", fontsize=14, fontweight='semibold', pad=20)
    ax.set_xlabel("Indice de la Couche", fontsize=11, labelpad=10)
    ax.set_ylabel("Rang Effectif (Dimensions utilisées)", fontsize=11, labelpad=10)
    ...
    ax.legend(title="Modèles", loc="best", frameon=True)

    plt.tight_layout()
    
    output_dir = Path("output")
    output_dir.mkdir(exist_ok=True)
    output_png = output_dir / "une_comparison_rank.png"
