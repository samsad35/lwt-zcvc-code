...
def get_ultimate_matrices(X, k=128):
    # Pillar 4: Median Centering
    ...
    # Pillar 3: Strategic PCA Truncation
...
def main():
    ...
    # Pillar 1: Asymmetric Global Pooling (Target uses Pool)
    X_tgt_pool = np.concatenate(X_tgt_list, axis=0)
    
    N_COMPONENTS = 128
    ALPHA = 0.85 # Pillar 2: Soft Coloring
    
    mu_tgt, _, C_tgt = get_ultimate_matrices(X_tgt_pool, k=N_COMPONENTS)
    ...
    print(f"\n3. Running Ultimate WCT (Dim={N_COMPONENTS}, Alpha={ALPHA})...")
    ...
    for p in tqdm(src_paths, desc="Evaluating Ultimate WCT"):
        X_src, w_src = extract_features(p, wavlm_large)
        ref_text = transcribe(w_src)
        
        # Pillar 1: Asymmetric Global Pooling (Source is Local)
        k_src = min(N_COMPONENTS, X_src.shape[0] - 1)
        mu_src, W_src, C_src = get_ultimate_matrices(X_src, k=k_src)
        
        # Pillar 2: Soft Coloring Interpolation
        C_interp = ALPHA * C_tgt + (1 - ALPHA) * C_src
        
        # Transform
        X_whitened = np.matmul(X_src - mu_src, W_src)
        X_colored = np.matmul(X_whitened, C_interp) + mu_tgt
