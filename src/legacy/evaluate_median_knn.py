...
def main():
    ...
    print("\n3. Running MEDIAN-FILTERED LinearVC Pipeline...")
    ...
    KERNEL_SIZE = 5 # Median filter window size
    
    for p in tqdm(src_paths, desc="Evaluating Median-kNN"):
        X_src, w_src = extract_features(p, wavlm_large)
        ref_text = transcribe(w_src)
        
        # 1. Standard kNN
        _, indices = knn.kneighbors(X_src)
        indices = indices.squeeze()
        
        # 2. MEDIAN FILTERING ON INDICES !
        # This removes isolated glitches but preserves legitimate jumps (step edges)
        filtered_indices = medfilt(indices, kernel_size=KERNEL_SIZE).astype(int)
        
        # 3. Retrieve pure filtered target frames
        X_tgt_matched = X_tgt_pool[filtered_indices]
    ...
    print(f"\n================ MEDIAN-FILTERED KNN RESULTS (Kernel={KERNEL_SIZE}) ================")
