...
def extract_features(wavlm_target, wavlm_source, audio_paths, l_target, l_source, desc_msg):
    utt_target = []
    utt_source = []
    speaker_ids = []
    ...
    with torch.no_grad():
        for path in tqdm(audio_paths, desc=desc_msg):
            ...
            # Mean Pooling for Speaker ID
            utt_target.append(np.mean(h_t, axis=0))
            utt_source.append(np.mean(h_s, axis=0))
            speaker_ids.append(get_speaker_id(path))
            
    return np.array(utt_target), np.array(utt_source), np.array(speaker_ids)
...
def main():
    ...
    X_train, Y_train, y_train = extract_features(wavlm_base, wavlm_asr, probe_train_paths, L_TARGET, L_SOURCE, "Probe Train Extraction")
    X_test, Y_test, y_test = extract_features(wavlm_base, wavlm_asr, probe_test_paths, L_TARGET, L_SOURCE, "Probe Test Extraction")
    ...
    clf_recon = LogisticRegression(max_iter=1000, random_state=42).fit(scaler_recon.transform(X_train_recon), y_train)
    ...
    plt.title("Probing Identité Locuteur : Désenchevêtrement par Finetuning ASR", 
              fontsize=14, fontweight='bold', pad=15)
    
    out_file = output_dir / "cca_speaker_wavlm_asr.png"
